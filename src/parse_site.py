...
class SiteParser:
    def __init__(self, vector_client=None):
        ...
        self.collection_configs = {
            "719": "Консультации по 719-ПП. Задача модуля: помочь пользователю определить корректный ОКПД2 и понять, какие требования/операции нужно подтверждать, а также какие документы нужны, с учётом того, выполняются операции на собственной площадке или у подрядчика (контрактное производство).",
            "support": "Меры поддержки бизнеса. Задача модуля: по параметрам компании и запросу клиента подобрать релевантные меры поддержки (субсидии, займы, льготы, программы).",
            "services": "Услуги ТПП. Задача модуля: по описанию проблемы пользователя предложить подходящие услуги ТПП и маршрут решения.",
            "membership": "Членство в ТПП. Задача: отвечать на вопросы о членстве, преимуществах и порядке вступления, с персонализацией под запрос клиента.",
            "events": "Мероприятия, обучение. Задача: по запросу пользователя выдавать список ближайших мероприятий и ссылку на регистрацию.",
            "cooperation": "Поиск партнёров / коопераций. Задача: помочь пользователю найти партнёров/поставщиков/подрядчиков среди членов ТПП УР и, при необходимости, уточнить профиль компаний по открытым источникам.",
            "site": "Общий контент сайта. Здесь хранится информация, не подходящая под другие категории.",
        }
